...
(defun collapse-updirs (directory)
  ;; (:relative "git" :up "assets") -> (:relative "assets")
  (let ((result '()))
    (dolist (part directory (nreverse result))
      (if (and (eq part :up) (stringp (first result)))
          (pop result)
          (push part result)))))
  ...
  (if (or (null *link-prefix*) (external-link-p url))
      ...
      (let* ((hash (position #\# url))
             ...
             (path (if (equal (pathname-type path) "md")
                       ...
                       path))
             (path (make-pathname :directory (collapse-updirs (pathname-directory path))
                                  :defaults path)))
